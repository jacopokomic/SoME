...
<div style="font-family: 'JetBrains Mono', 'SF Mono', 'Cascadia Code', monospace; font-size: 14px; color: var(--text, #0f172a); padding: 10px 0 6px 0;">
    Training the network: epoch {epoch}/{epochs}
    &mdash; validation AUC <b style="color: var(--accent, #3b82f6);">{auc:.4f}</b>
</div>
...
def track(epoch, epochs, val_auc):
...
def predict():
    ...
    sliders = {
        ...
        "income": widgets.IntSlider(value = 5000, min = 0, max = 30000, step = 100, description = "Monthly income in $", style = style, layout = layout),
